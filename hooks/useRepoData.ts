...
import { FileNode, RepoMetadata, ParsedRepo } from '../app/analyzer/types';
import { useGithubClone } from '@/hooks/useGithubClone';
import { sdkList } from '@/data/sdkData';
...
export function useRepoData(repoPath: string) {
  ...
  const getLanguageModeForFile = (filename: string): string => {
  ...
  useEffect(() => {
    ...
    analyzeRepository()
  }, [repoPath, parseRepositoryUrl, fetchRepoMetadata, cloneRepository, calculateRepoStats, findReadmeFile, cleanFileTree]);
